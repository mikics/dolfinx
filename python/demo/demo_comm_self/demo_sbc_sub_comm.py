...
# +
num_sub_comm = 4
color = MPI.COMM_WORLD.rank % num_sub_comm
key = int(MPI.COMM_WORLD.rank / num_sub_comm)

sub_comm = MPI.COMM_WORLD.Split(color=color,
                                key=key)

print(f"color: {color}, rank: {MPI.COMM_WORLD.rank}, sub_rank: {key}")
model = None
...
model = sub_comm.bcast(model, root=0)
domain, cell_tags, facet_tags = io.gmshio.model_to_mesh(
    model, sub_comm, 0, gdim=2)
...
wl0_arr = np.linspace(0.4, 0.8, 10)
...
for n in range(int(ceil(wl0_arr.shape[0] / num_sub_comm))):
    ...
    if n * num_sub_comm + color < wl0_arr.shape[0]:
        ...
        eps.x.array[bkg_cells] = np.full_like(
            bkg_cells, eps_bkg, dtype=np.complex128)
