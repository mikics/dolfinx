...
for m in m_list:
    ...
    V_r, r_dofs = Esh_rz_m_dg.function_space.sub(0).collapse()
    V_z, z_dofs = Esh_rz_m_dg.function_space.sub(1).collapse()
    V_p, p_dofs = V.sub(1).collapse()

    Esh_r_dg = fem.Function(V_r)
    Esh_z_dg = fem.Function(V_z)
    Esh_p = fem.Function(V_p)

    if m == 0:

        Esh_r_dg.x.array[:] = Esh_rz_m_dg.x.array[r_dofs]

        Esh_z_dg.x.array[:] = Esh_rz_m_dg.x.array[z_dofs]

        Esh_p.x.array[:] = Esh_p_m.x.array[p_dofs]

    elif m == m_list[0]:

        Esh_r_dg.x.array[:] = Esh_rz_m_dg.x.array[r_dofs] * \
            (np.exp(1j * m * phi) + np.exp(- 1j * m * phi))
    ...
    if m == 0:  # initialize and do not add 2 factor
        ...
        q_sca_fenics_proc_plus = (fem.assemble_scalar(
            fem.form((P('+')) * rho * dS(scatt_tag))) / gcs / I0).real

        q_sca_fenics_proc_minus = (fem.assemble_scalar(
            fem.form((P('-')) * rho * dS(scatt_tag))) / gcs / I0).real

        print(q_sca_fenics_proc_minus)
        print(q_sca_fenics_proc_plus)
...
# +
with VTXWriter(domain.comm, "sols/Es_r.bp", Esh_r_dg) as f:
    f.write(0.0)
with VTXWriter(domain.comm, "sols/Es_z.bp", Esh_z_dg) as f:
    f.write(0.0)
