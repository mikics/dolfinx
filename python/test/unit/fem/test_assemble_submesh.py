...
def assemble_mixed_forms(comm, f, g, h, u, v, dx, ds):
    # a = fem.form(ufl.inner(f * g * u, v) * (dx + ds) + ufl.inner(h * u, v) * dx)
    a = fem.form(ufl.inner(h * u, v) * dx)
    A = fem.petsc.assemble_matrix(a)
    A.assemble()

    # L = fem.form(ufl.inner(f * g, v) * (dx + ds) + ufl.inner(h, v) * dx)
    L = fem.form(ufl.inner(h, v) * dx)
    b = fem.petsc.assemble_vector(L)
    b.ghostUpdate(addv=PETSc.InsertMode.ADD,
                  mode=PETSc.ScatterMode.REVERSE)

    # M = fem.form(f * g * (dx + ds) + h * dx)
    M = fem.form(h * dx)
...
def test_mixed_codim_0_assembly(d, n, k, space, ghost_mode):
    ...
    submesh_2, entity_map, vertex_map, geom_map = create_submesh(
    ...
    A_sm, b_sm, s_sm = assemble_mixed_forms(
        submesh.comm, f_m1, g_sm, h_sm_2, u_sm, v_sm, dx_sm, ds_sm)
